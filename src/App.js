...
class App extends Component {
    ...
    if (!(data.branches && data.rowData)) {
      ...
      if (data.newick || data.newickjs) {
        ...
        const taxa = Object.keys(data.rowData).sort(),
          seqs = taxa.map((taxon) => data.rowData[taxon])
        console.warn(
          'Estimating phylogenetic tree (' + taxa.length + ' taxa)...'
        )
  ...
  unpackStockholmJS(data, config, stock) {
    ...
    if (stock.gf.DE) data.description = stock.gf.DE.join('\n')
      ...
      Object.keys(stock.gs.DR).forEach((node) => {
        ...
        stock.gs.DR[node].forEach((dr) => {
          ...
          if (match) {
            ...
            if (startPos <= seqCoords.endPos && endPos >= seqCoords.startPos) {
              // check structure overlaps sequence
              structure[node] = structure[node] || { pdb, chains: [] }
              structure[node].chains.push({ chain, startPos, endPos })
            } else
              console.warn(
                'ignoring structure ' +
                  pdb +
                  ...
                  '...' +
                  endPos +
                  ') since it does not overlap with ' +
                  node
              )
          }
        })
  ...
  async initDataset() {
    if (this.props.stockholm) this.addDatasets(this.props.stockholm)
    if (this.props.dataurl)
      await fetch(this.makeURL(this.props.dataurl)).then(async (res) => {
        if (res.ok) this.addDatasets(await res.text())
      })
  ...
  buildTreeIndex(data) {
    const { branches } = data
    let { root } = data,
      rootSpecified = typeof root !== 'undefined'
