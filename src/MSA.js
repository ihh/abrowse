...
class MSA extends Component {
  constructor(props) {
    ...
    this.state = extend({ scrollTop: 0, alignScrollLeft: 0 }, { view })
  ...
  getComputedView(view) {
    ...
    const setCollapsedState = (node, parent) => {
      ...
      if (kids) kids.forEach((child) => setCollapsedState(child, node))
    }
  ...
  layoutAlignment(computedView) {
    ...
    alignChars.forEach((c) => {
      let measureCanvas = this.create('canvas', null, null, {
  ...
  handleNodeClick(node) {
    ...
    if (this.state.disableTreeEvents) return
    ...
    const alignLayout = this.layoutAlignment(computedView)
    const left = this.state.alignScrollLeft,
      right = left + this.alignmentClientWidth
    ...
    const wasCollapsed = collapsed[node],
      finalCollapsed = extend({}, collapsed)
    if (wasCollapsed) {
    ...
    for (let col = 0; col < alignIndex.columns; ++col) {
      ...
      const colX = alignLayout.colX[col],
        colWidth = alignLayout.colWidth[col]
      if (
        columnVisible[col] &&
    ...
    const drawAnimationFrame = () => {
      ...
      const computedView = this.getComputedView(view),
        alignLayout = this.layoutAlignment(computedView)
  ...
  handleMouseWheel(evt) {
    ...
    if (evt.deltaY !== 0) {
      ...
      this.requestAnimationFrame(() => {
        this.setState({
          alignScrollLeft: this.incAlignScrollLeft(evt.deltaX),
  ...
  handleMouseMove(evt) {
    if (this.alignMouseDown || this.mousedown) evt.preventDefault()
